...
namespace CrossHandlers
{
	...
	class IDXTexWrite
	{
		...
		public:
			...
			virtual bool CreateShaderView(D3D10_SHADER_RESOURCE_VIEW_DESC* desc) = 0;
			///<summary>Creates a second shaderview from the underlying texture for reading the stencil of a depthbuffer</summary>
			///<param name='desc'>[in]The description to create the shader from</param>
			///<returns>If the function succeded</returns>
			virtual bool CreateSecondShaderView(D3D10_SHADER_RESOURCE_VIEW_DESC* desc) = 0;
			...
			///<returns>If the function succeded</returns>
			virtual bool CreateTargetView(D3D10_RENDER_TARGET_VIEW_DESC* desc) = 0;
			...
			///<returns>If the function succeded</returns>
			virtual bool CreateDepthView(D3D10_DEPTH_STENCIL_VIEW_DESC* desc) = 0;
